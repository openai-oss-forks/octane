...
const slots = Object.fromEntries(
	['cancelToken', 'excludeKeys', 'pendingOptimistic', 'submit', 'cancel', 'submitMethods'].map(
		(name) => [name, Symbol(`Inertia.useForm.${name}`)],
	),
...
export type InertiaPrecognitiveFormProps<TForm extends Record<string, any>> =
	InertiaFormProps<TForm> & InertiaFormValidationProps<TForm>;
...
export default function useForm<TForm extends FormDataType<TForm>>(
	...
	const publicArgs =
		typeof args[args.length - 1] === 'symbol'
			? (args.slice(0, -1) as UseFormArguments<TForm>)
			: args;
	const { rememberKey, data, precognitionEndpoint } = UseFormUtils.parseUseFormArguments<TForm>(
		...publicArgs,
	);
	...
	const cancelToken = useRef<CancelToken | null>(null, slots.cancelToken);
	const excludeKeysRef = useRef<FormDataKeys<TForm>[]>([], slots.excludeKeys);
	const pendingOptimisticRef = useRef<OptimisticCallback | null>(null, slots.pendingOptimistic);
	...
	const submit = useCallback(
		(...args: UseFormSubmitArguments) => {
			const { method, url, options } = UseFormUtils.parseSubmitArguments(
				args,
				precognitionEndpointRef.current,
			);
