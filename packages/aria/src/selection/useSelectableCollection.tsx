...
// Ported from adobe/react-spectrum@5ecb3333001313e83898cd07644227897e3bae1f (packages/react-aria/src/selection/useSelectableCollection.ts).
/*
...
export function useSelectableCollection(
	...
	let withShiftSel = (
		key: string,
		callback: import('../interactions/createKeyboardShortcutHandler').KeyboardShortcutAction,
	) => {
		return {
	...
	let onBlur = (e: globalThis.FocusEvent) => {
		// Don't set blurred and then focused again if moving focus within the collection.
		if (!nodeContains(e.currentTarget as Element, e.relatedTarget as HTMLElement)) {
			manager.setFocused(false);
		}
	};
	...
	let lastFocusedKey = useRef<typeof manager.focusedKey>(manager.focusedKey);
	...
	let handlers = {
		...
		onMouseDown(e: MouseEvent) {
			...
			if (scrollRef.current === (getEventTarget(e) as Element)) {
