...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-aria/src/color/useColorAreaGradient.ts).
/*
		...
		switch (state.value.getColorSpace()) {
			...
			case 'hsl': {
				...
				let bg = channels
					...
					.map(
						(c) =>
							`linear-gradient(to ${c === xChannel ? end : 'top'}, ${(hslChannels as Record<string, (color: Color) => string>)[c](value)})`,
					)
					.reverse();
				if (zChannel === 'hue') {
			...
			case 'hsb': {
				...
				let bg = channels
					...
					.map(
						(c) =>
							`linear-gradient(to ${c === xChannel ? end : 'top'}, ${(hsbChannels as Record<string, (color: Color) => string>)[c](value)})`,
					)
					.reverse();
				if (zChannel === 'hue') {
					bg.push(value.toString('css'));
