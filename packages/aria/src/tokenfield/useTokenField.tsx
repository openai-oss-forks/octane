...
// Ported from adobe/react-spectrum@5ecb3333001313e83898cd07644227897e3bae1f (packages/react-aria/src/tokenfield/useTokenField.ts).
/*
...
export interface AriaTokenFieldProps<T extends TokenFieldValue = TokenFieldValue>
	...
	onKeyDown?: (e: import('../interactions/createEventHandler').BaseEvent<KeyboardEvent>) => void;
	/** Handler that is called when a key is released. */
	onKeyUp?: (e: import('../interactions/createEventHandler').BaseEvent<KeyboardEvent>) => void;
	/**
...
export function useTokenField<T extends TokenFieldValue = TokenFieldValue>(
	...
	let moveSelection = (
		...
		while (true) {
			const focusNode: globalThis.Node | null = selection.focusNode;
...
function useSelectionChange(ref: React.RefObject<Element | null>, handler: () => void) {
	useEvent(useRef(typeof document !== 'undefined' ? document : null), 'selectionchange', () => {
		if (
			ref.current &&
			(ref.current as Element & { [isProgrammaticSelectionChange]?: boolean })[
				isProgrammaticSelectionChange
			]
		) {
			(ref.current as Element & { [isProgrammaticSelectionChange]?: boolean })[
