...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-aria/src/toast/useToastRegion.ts).
/*
	...
	useLayoutEffect(() => {
		...
		if (removedFocusedToastIndex > -1) {
			...
			if (getInteractionModality() === 'pointer' && lastFocused.current?.isConnected) {
				...
				if (prevToast != null && prevToast >= 0 && prevToast < toasts.current.length) {
					focusWithoutScrolling(toasts.current[prevToast]);
				} else if (nextToast != null && nextToast >= 0 && nextToast < toasts.current.length) {
