...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-aria/src/dnd/useDroppableCollection.ts).
/*
...
export function useDroppableCollection(
	...
	let { dropProps } = useDrop({
		...
		getDropOperationForPoint(types, allowedOperations, x, y) {
			...
			let isInternal = isInternalDropOperation(ref);
			let isValidDropTarget = (target: DropTarget) =>
	...
	let droppingState = useRef<DroppingState | null>(null);
			...
			if (
				...
				if (state.selectionManager.focusedKey === prevFocusedKey) {
					...
					if (first != null) {
						...
						let isParentRowExpanded = (state.collection as any).expandedKeys
							? (state.collection as any).expandedKeys.has(item?.parentKey)
							: false;
