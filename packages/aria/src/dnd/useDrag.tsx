...
export function useDrag(options: DragOptions): DragResult {
	...
	let modalityOnPointerDown = useRef<string | null>(null);
	...
	let message = !isDragging
		? MESSAGES[modality as keyof typeof MESSAGES].start
		: MESSAGES[modality as keyof typeof MESSAGES].end;
	...
	if (!hasDragButton) {
		...
		interactions = {
			...
			onKeyUpCapture(e: KeyboardEvent<HTMLElement>) {
				if (getEventTarget(e) === e.currentTarget && e.key === 'Enter') {
					e.preventDefault();
					e.stopPropagation();
					startDragging(getEventTarget(e) as HTMLElement);
