...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-aria/src/dnd/useDraggableItem.ts).
/*
...
export function useDraggableItem(
	...
	if (!props.hasDragButton && state.selectionManager.selectionMode !== 'none') {
		let msg = MESSAGES[modality as keyof typeof MESSAGES][isSelected ? 'selected' : 'notSelected'];
	...
	if (!props.hasDragButton && props.hasAction) {
		...
		dragProps.onKeyDownCapture = (e: KeyboardEvent<HTMLElement>) => {
			if (e.altKey) {
				onKeyDownCapture?.(e);
			}
		};

		dragProps.onKeyUpCapture = (e: KeyboardEvent<HTMLElement>) => {
			if (e.altKey) {
				onKeyUpCapture?.(e);
			}
		};
