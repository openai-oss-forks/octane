...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-stately/src/data/useTreeData.ts).
/*
...
export function useTreeData<T extends object>(options: TreeOptions<T>): TreeData<T> {
	...
	function updateTree(
		...
		while (node && node.parentKey) {
			...
			if (newNode == null && children) {
				children = children.filter((c: TreeNode<T>) => c !== node);
			}
	...
	return {
		...
		remove(...keys: Key[]) {
			...
			let newItems = items;
			let prevMap = nodeMap;
			let newTree: TreeDataState<T> = { items: newItems, nodeMap: prevMap };
			for (let key of keys) {
...
function moveItems<T extends object>(
	...
	function inorder(child: TreeNode<T>) {
	...
	function postorder(child: TreeNode<T>) {
