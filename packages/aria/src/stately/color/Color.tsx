...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-stately/src/color/Color.ts).
/*
...
abstract class Color implements IColor {
	...
	getChannelName(channel: ColorChannel, locale: string) {
		...
		return strings.getStringForLocale(channel, locale) as string;
	}
	...
	getColorName(locale: string): string {
		...
		let strings =
			LocalizedStringDictionary.getGlobalDictionaryForPackage('@react-stately/color') || dictionary;
		if (l > 0.999) {
			return strings.getStringForLocale('white', locale) as string;
		}

		if (l < 0.001) {
			return strings.getStringForLocale('black', locale) as string;
		}
		...
		if (chroma) {
			chroma = strings.getStringForLocale(chroma, locale) as string;
		}

		if (lightness) {
			lightness = strings.getStringForLocale(lightness, locale) as string;
		}
	...
	private getOklchHue(l: number, c: number, h: number, locale: string): [string, number] {
		let strings =
			LocalizedStringDictionary.getGlobalDictionaryForPackage('@react-stately/color') || dictionary;
		if (c < GRAY_THRESHOLD) {
			return [strings.getStringForLocale('gray', locale) as string, l];
		}
		...
		for (let i = 0; i < OKLCH_HUES.length; i++) {
			...
			if (h >= hue && h < nextHue) {
				...
				let name = (strings.getStringForLocale(hueName, locale) as string).toLocaleLowerCase(
					locale,
				);
				return [name, l];
			}
