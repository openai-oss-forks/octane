...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-stately/src/datepicker/useDateFieldState.ts).
/*
...
export function useDateFieldState<T extends DateValue = DateValue>(
	...
	let placeholder = useMemo(
		...
		[props.placeholderValue, granularity, calendar, defaultTimeZone],
	);
	let displaySegments = useMemo(() => {
		let is12HourClock = hourCycle === 'h11' || hourCycle === 'h12';
		let segments: DateSegmentType[] = [
			'era',
			'year',
			'month',
			'day',
	...
	let adjustSegment = (type: DateSegmentType, amount: number) => {
	...
	return {
		...
		incrementPage(part) {
			adjustSegment(part, (PAGE_STEP as Record<string, number>)[part] || 1);
		},
		decrementPage(part) {
			adjustSegment(part, -((PAGE_STEP as Record<string, number>)[part] || 1));
		},
		incrementToMax(part) {
			let maxValue =
				part === 'hour' && hourCycle === 'h12' ? 11 : displayValue.getSegmentLimits(part)!.maxValue;
			setValue(displayValue.set(part, maxValue, placeholder));
		},
		decrementToMin(part) {
			let minValue =
				part === 'hour' && hourCycle === 'h12' ? 12 : displayValue.getSegmentLimits(part)!.minValue;
...
function processSegments(
	...
	let twoDigitFormatter = new NumberFormatter(locale, {
		useGrouping: false,
	...
	for (let segment of segments) {
		let type = (TYPE_MAPPING as Record<string, DateSegmentType>)[segment.type] || segment.type;
		let isEditable = (EDITABLE_SEGMENTS as Record<string, boolean>)[type];
		if (type === 'era' && calendar.getEras().length === 1) {
			isEditable = false;
		}

		let isPlaceholder =
			(EDITABLE_SEGMENTS as Record<string, boolean>)[type] && displayValue[segment.type] == null;
		let placeholder = (EDITABLE_SEGMENTS as Record<string, boolean>)[type]
