...
// Ported from adobe/react-spectrum@1c84a49a1faf50b571c84e00bcf9c60b22ddd03e (packages/react-stately/src/datepicker/IncompleteDate.ts).
/*
...
export class IncompleteDate {
	...
	isComplete(segments: DateSegmentType[]) {
		return segments.every((segment) => (this as any)[segment] != null);
	}
	...
	validate(dt: DateValue, segments: DateSegmentType[]) {
		return segments.every((segment) => {
			...
			return (this as any)[segment] === (dt as any)[segment];
		});
	}
	...
	cycle(
		...
		if ((res as any)[field] == null && field !== 'dayPeriod' && field !== 'era') {
			if (field === 'hour' && 'hour' in placeholder) {
				...
				(res as any)[field] = (placeholder as any)[field];
			}
