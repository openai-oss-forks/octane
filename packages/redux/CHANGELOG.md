...
- ce3a6fe: Fix published source that a browser application cannot type-check.
...
- dcd2707: Fix `useSyncExternalStoreWithSelector` calls that omit `isEqual` so the compiler's trailing hook slot is not interpreted as the optional comparator.
