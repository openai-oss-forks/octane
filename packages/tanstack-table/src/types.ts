...
import type { Atom, ReadonlyAtom, ReadonlyStore, Store } from '@octanejs/tanstack-store';
import type {
	AccessorFn,
	AccessorFnColumnDef,
 ...
 * {@link TableState}.
 */
export interface SubscribePropsWithStore<TFeatures extends TableFeatures, TSelected> {
...
export interface SubscribePropsWithSourceIdentity<TSourceValue> {
	...
	selector?: undefined;
	children: ((state: TSourceValue) => OctaneNode) | OctaneNode;
}
...
export interface SubscribeComponent {
	<TSourceValue>(props: SubscribePropsWithSourceIdentity<TSourceValue>): OctaneNode;
...
> = Omit<Table<TFeatures, TData>, 'store'> & {
	...
	Subscribe: {
		...
		<TSourceValue, TSubSelected>(props: {
			...
			children: ((state: TSubSelected) => OctaneNode) | OctaneNode;
		}): OctaneNode;
		<TSubSelected>(
 ...
 */
export interface TableHookContexts<TFeatures extends TableFeatures, TData extends RowData> {
	tableContext: Context<OctaneTable<any, any>>;
	cellContext: Context<Cell<any, any, any>>;
	headerContext: Context<Header<any, any, any>>;
	useTableContext: <TTableData extends RowData = TData>() => OctaneTable<TFeatures, TTableData>;
...
> {
	cell: Cell<TFeatures, TData, TValue> & TCellComponents & { FlexRender: () => OctaneNode };
...
> {
	column: Column<TFeatures, TData, TValue>;
	header: Header<TFeatures, TData, TValue> & THeaderComponents & { FlexRender: () => OctaneNode };
	table: Table<TFeatures, TData>;
}
...
export type AppColumnDefBase<
	...
	TCellComponents extends Record<string, TableComponentType>,
	THeaderComponents extends Record<string, TableComponentType>,
> = Omit<IdentifiedColumnDef<TFeatures, TData, TValue>, 'cell' | 'header' | 'footer'> & {
...
export type AppDisplayColumnDef<
	...
	TCellComponents extends Record<string, TableComponentType>,
	THeaderComponents extends Record<string, TableComponentType>,
> = Omit<DisplayColumnDef<TFeatures, TData, unknown>, 'cell' | 'header' | 'footer'> & {
...
export type AppGroupColumnDef<
	...
	TCellComponents extends Record<string, TableComponentType>,
	THeaderComponents extends Record<string, TableComponentType>,
> = Omit<GroupColumnDef<TFeatures, TData, unknown>, 'cell' | 'header' | 'footer' | 'columns'> & {
	cell?: AppColumnDefTemplate<AppCellContext<TFeatures, TData, unknown, TCellComponents>>;
	header?: AppColumnDefTemplate<AppHeaderContext<TFeatures, TData, unknown, THeaderComponents>>;
	footer?: AppColumnDefTemplate<AppHeaderContext<TFeatures, TData, unknown, THeaderComponents>>;
...
> {
	...
	>(
		...
		column: TAccessor extends AccessorFn<TData>
			? AppColumnDefBase<TFeatures, TData, TValue, TCellComponents, THeaderComponents> & {
					id: string;
				}
			: AppColumnDefBase<TFeatures, TData, TValue, TCellComponents, THeaderComponents>,
	 ...
	 */
	display: (
		column: AppDisplayColumnDef<TFeatures, TData, TCellComponents, THeaderComponents>,
	 ...
	 */
	group: (
		column: AppGroupColumnDef<TFeatures, TData, TCellComponents, THeaderComponents>,
...
export type CreateTableHookOptions<
	...
	TCellComponents extends Record<string, TableComponentType>,
	THeaderComponents extends Record<string, TableComponentType>,
> = Omit<TableOptions<TFeatures, any>, 'columns' | 'data' | 'store' | 'state' | 'initialState'> & {
	/**
...
> {
	...
	<TValue extends CellData = CellData, TSelected = unknown>(
		props: AppCellPropsWithSelector<TFeatures, TData, TValue, TCellComponents, TSelected>,
	): OctaneNode;
}
...
> {
	...
	<TValue extends CellData = CellData, TSelected = unknown>(
		props: AppHeaderPropsWithSelector<TFeatures, TData, TValue, THeaderComponents, TSelected>,
	): OctaneNode;
}
...
export interface AppTableComponent<TFeatures extends TableFeatures> {
	...
	<TSelected>(props: AppTablePropsWithSelector<TFeatures, TSelected>): OctaneNode;
}
