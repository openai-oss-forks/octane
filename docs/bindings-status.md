...
| [`@octanejs/jotai`](#octanejsjotai) | `jotai@3.0.0` | Jotai 3 runtime surface: imported vanilla atoms, stores, utilities and internals; ported Provider, useStore, useAtom, useSetAtom, useAtomValue, useAtomValueRaw, useAtomValueRawSync and four hook utilities. | Octane hook call-site slots are forwarded through composed hooks; React class error-boundary test fixtures use Octane functional ErrorBoundary components; Commit-count test hooks run in component setup and use explicit every-render effects to preserve the upstream observation boundary | Server rendering and hydration are tested for both raw hooks with request-local stores, no server subscriptions, adopted DOM, client updates and teardown. | 2026-09-12 |
...
- Use additional Anime.js subpaths directly from animejs; the root and adapters/three convenience exports remain supported.
...
- Octane hook call-site slots are forwarded through composed hooks.
- React class error-boundary test fixtures use Octane functional ErrorBoundary components.
- Commit-count test hooks run in component setup and use explicit every-render effects to preserve the upstream observation boundary.

SSR / hydration: Server rendering and hydration are tested for both raw hooks with request-local stores, no server subscriptions, adopted DOM, client updates and teardown.
...
- Generated wrappers consume official framework-neutral @lucide/icons@1.45.0 data and its public build export. The Octane bridge supports icon data, nested nodes, custom view boxes, and non-scaling strokes without copying new React implementation.
- Generation checks pin the React export, alias, and dynamic-name surfaces and reject stale generated files.
- Two shared runtime differential scenarios plus complete canonical SVG comparisons, direct public type probes, hydration, native refs/events and dynamic-import lifecycle tests cover the current dependency. The historical upstream runtime suite remains unvendored and unadapted one-for-one.
- Every pinned upstream test artifact and public entry-point family is dispositioned in UPSTREAM.md.
- Release identities are recorded in audit/release-compatibility.json. Historical ISC source attribution remains recorded-unverified; the current dependency update does not claim full upstream-suite parity.
...
- The 5.2.2 release only names component functions and changes declaration export printing. All 236 generated subpaths remain byte-identical, and declaration bodies/export identities are unchanged. See audit/release-compatibility.json.
