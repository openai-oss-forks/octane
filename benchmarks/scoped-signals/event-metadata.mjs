...
export async function measureEventMetadata({
	runtimePath,
	dev = false,
	rows = 100,
	updates = 4,
...
export function mount(parent,props) {
 ...
 return {update(props){flushSync(()=>root.render(List,props));},dispose(){root.unmount();}};
	...
	try {
		...
		compiled = compile(authored, path.join(root, 'benchmarks/scoped-signals/EventMetadata.tsrx'), {
			mode: 'client',
			dev,
			hmr: false,
		});
	...
	const count = (callback) => {
		records = 0;
		// Observe journal Map insertions for the existing nominal native bundles.
		// Each insertion snapshots that bundle once in the current journal window.
		// This is operation evidence; callback correctness is checked separately.
		Map.prototype.set = function (key, value) {
			if (
				key !== null &&
				typeof key === 'object' &&
				typeof key.fn === 'function' &&
				key.el instanceof window.Element &&
				Object.hasOwn(key, 'args')
			)
				records++;
	...
	try {
		...
		const changed = count(() => mounted.update({ items, invoke, other: 'second' }));
...
if (process.argv[1] && path.resolve(process.argv[1]) === path.resolve(import.meta.filename)) {
	console.log(
		JSON.stringify(await measureEventMetadata({ runtimePath: process.argv[2] }), null, 2),
	);
}
